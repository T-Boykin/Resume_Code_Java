...
public class CharAttributes
{
  ...
  int health, player;
  float gravity;
  float jump, walkspeed, dash, pushback, stuntime;
  Rectangle chardims;
  ...
  public CharAttributes(String character, int total, int device, AndroidInterfaceLIBGDX adr)
  {

    if (total != 3)
    {
      adr.Open();
      adr.charQuery(character);

      chardims = new Rectangle(adr.getDimX(), adr.getDimY(), adr.getDimW(), adr.getDimY());
      health = adr.getHealth();
      gravity = adr.getGrav();
      jump = adr.getJump();
      walkspeed = adr.getWalkspeed();
      wandj = new Vector2(walkspeed, 0);
      dash = adr.getDash();
      tempplayer = adr.retAtkInfo(character);
      adr.retAllAnims();
      anims = adr.retAnimCodes();
      animations = adr.retAnimData();
      animFPS = adr.retFPS();
      moveset = adr.retMoves();
      inputs = adr.Inputs();
      adr.Close();
    }
    // Read the file that will contain the frame information
  ...
  public List<ArrayList<Double>> process(Connection c, String chr, List<ArrayList<Double>> t)
  {
    ...
    ArrayList<Double> dimX = new ArrayList<Double>();
    ArrayList<Double> dimY = new ArrayList<Double>();
    ArrayList<Double> dimW = new ArrayList<Double>();
    ArrayList<Double> dimH = new ArrayList<Double>();
    ArrayList<Double> spawn = new ArrayList<Double>();
    ...
    String proj_dimx = "spfzdimx";
    String proj_dimy = "spfzdimy";
    String proj_dimw = "spfzdimw";
    String proj_dimh = "spfzdimh";
    String proj_spwn = "spawnfrm";
    ...
    try
    {
      ...
      while (set.next())
      {
        ...
        if (isProjectile(set.getString(atk_atk)))
        {
          ...
          dimW.add(set.getDouble(proj_dimw));
          dimH.add(set.getDouble(proj_dimh));
          dimX.add(set.getDouble(proj_dimx));
          dimY.add(set.getDouble(proj_dimy));
          spawn.add(set.getDouble(proj_spwn));
  ...
  public Connection dbconnect()
  {
    ...
    //url = "jdbc:sqlite:S:/SPFZPROJBACKUP/spfzalpha/android/assets/databases/spfz.sqlite";

   /* switch (Gdx.app.getType())
    {
      case Android:
        url = "jdbc:sqlite:" + Gdx.files.getLocalStoragePath() + "/charinfo/spfzsqlite";
        ;
        break;
      case Desktop:
        url = "jdbc:sqlite:C:/SPFZ Code/spfzalpha/android/assets/charinfo/spfz.sqlite";
        break;
      default:
        url = "jdbc:sqlite:C:/SPFZ Code/spfzalpha/android/assets/charinfo/spfz.sqlite";
        break;
